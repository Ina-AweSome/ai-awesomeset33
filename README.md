...
- **メモは GoodNotes へ** — `📝 メモ` は GoodNotes に飛ぶ（Bear への依存は削除済み）。

## メモの飛び先（GoodNotes）

GoodNotes は「特定ノート（本日のノート）」を URL で直接開く仕組みを公式には提供して
いません（URL スキームは新規ノート作成・インポート系とアプリ起動のみ）。
そのため `public/app.js` 上部の `MEMO` で2モードを切り替えます。

| mode | 挙動 |
| --- | --- |
| `goodnotes`（既定） | `goodnotes://` で GoodNotes を開く。本日のノートは手動で選択。 |
| `shortcut` | `shortcuts://run-shortcut?name=…` で iOS ショートカットを実行。**本日のノートを確実に開く/追記する唯一の実用手段。** |

「本日のノート」に確実に飛ばしたい場合は `mode: 'shortcut'` にし、GoodNotes の当日ノートを
開く（必要ならカード内容を追記する）ショートカットを作って `shortcutName` に設定してください。
ショートカットには `text` としてカードのタイトル・本文・キーワードが渡ります。
